...
const MobileNavbar = () => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-50 bg-[#49BBBD] text-white shadow-md">
        ...
        <div className="fixed inset-0 z-50 flex ">
          ...
          <div className="w-64 sm:w-80 bg-white text-black h-full py-6 px-5 shadow-xl relative">
            ...
            <nav className="mt-12 flex flex-col gap-4 font-medium text-lg">
              ...
              <details className='shadow-sm p-2 rounded-md'>
                ...
                <ul className="ml-4 mt-2 text-sm text-gray-700">
                  {menu.careers.map((item, i) => (
                    <li key={i}>
                      <Link
                        className='hover:text-black my-2'
                        href={`/${item.toLowerCase().replace(/\s+/g, '-')}`}
                        onClick={() => setSidebarOpen(false)}
                      >
                        {item}
              ...
              <Link href="/login" onClick={() => setSidebarOpen(false)}>Login</Link>
              <Link href="/register" onClick={() => setSidebarOpen(false)}>Signup</Link>
            </nav>
          </div>
        </div>
      )}
