...
const BlogNav = () => {
  ...
  return (
    ...
    <div className="sticky top-0 z-20 text-base mx-w-8xl flex text-black/50 font-semibold items-center w-[1600px] px-14 ">
      {/* Logo */}
      <div className=" pl-5 text-start font-bold">
        <Link href="/">
          <Image
      ...
      {/* Center Nav */}
      <div className="flex justify-end w-full gap-12 relative text-sm">

        <div className="hover:text-black/80" >
            <Link href={"/"} >Home</Link>
        </div>
        ...
        >
          <button className="hover:text-black/80">Courses</button>
      ...
      {/* User Image */}
        <div className="flex gap-2 mr-5">
            <div className='rounded-full w-8 h-8 bg-black/20 overflow-hidden'>
                    <Image  alt='User image' src='/images/blogerImage.png' width={50} height={0} />
            </div>
                <h2 className="text-black pt-1">Lina</h2>
                <div
                className="relative"
                ...
                >
                  ...
                  {open === "Profile" && <Dropdown items={menu.profile} />}
                </div>
        </div>
      </div>  
    </div>
...
const Dropdown = ({ items, title, linktext, link }: {title?: string, linktext?: string, link?: string, items:string[] }) => (
  <div className="absolute left-[-100] top-full pt-3  text-black shadow-lg  z-50 w-[320px]">
    <div className="bg-white/90 rounded-xl py-3">
    <div className="w-8/10 mx-auto py-5 ">
      ...
      <hr />
      <div className="py-2">
        <Link className="pl-7 text-black/80 hover:text-black/100 flex items-center gap-2  font-medium" href={link || ''}>{linktext} {link && <TiArrowRightThick className="text-xl"/>} </Link>
      </div>
    </div>
  </div>
);
